...
Standard boolean operators are defined for all {\em Boolean}  type values $x, y$:
...
\begin{align*}	
Primary ::= & ~Literal ~|~ ParExpression ~|~ NewObjectExpression ~|~ NewArrayExpression\\
	&\mid ~'this'~('.' Identifier~(DimExpr)^*)^*~(IdentifierSuffix)? \\
	&\mid ~Identifier~ (DimExpr)^* ~('.' ~Identifier ~(DimExpr)^*)^* ~(IdentifierSuffix)?\\
IdentifierSuffix ::=& ~('[]')^+ ~ '.class' ~|~Arguments ~|~ '.class' ~|~ '.this'  ~|~ '.super'~Arguments	\\
&~|~ '.at' Arguments ~|~ '.Composition()' ~|~ '.Start()' ~|~ '(!)' ~|~ '(?)' \\
Arguments ::=&~ '('~ ExpressionList? ~')'\\
ExpressionList ::=& ~Expression ~(',' Expression)^*\\
DimExpr ::=&~ '['~Expression~ ']'\\
Literal ::=& ~'Inf' ~|~ IntegerLiteral ~|~ FLOATING\_POINT\_LITERAL ~|~ CHARACTER\_LITERAL \\ 
  & |~ STRING\_LITERAL ~|~ BooleanLiteral ~|~ 'null'
\end{align*}
...
\subsection{Continuous Flow Expressions}
...
\caption[Urgent Distance Control]{Urgent Distance Control.}
\label{fig:distance}
\end{figure}
